...
func (u *UserController) GetUserByID(w http.ResponseWriter, r *http.Request) {
    ...
    var user User
    err = db.QueryRow("SELECT user_id, user_name, email, full_name, FROM users WHERE user_id = ?", userID).Scan(&user.UserID, &user.UserName, &user.Email, &user.FullName, &user.GoogleOAuth)
    if err != nil {
